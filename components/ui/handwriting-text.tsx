...
// jsDelivr CDN is used as default to ensure fast and reliable global delivery (avoiding raw.githubusercontent ISP blocks)
...
export function HandwritingText({
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Degrade gracefully if font path creation errors
    }
  }, [font, current]);
  ...
  return (
    <svg
      ...
      role="img"
      aria-label={current}
      className={["inline-block align-baseline", className].filter(Boolean).join(" ")}
      style={{
        height,
