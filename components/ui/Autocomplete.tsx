...
export function Autocomplete({
  ...
  return (
    <div className="flex flex-col gap-1.5 w-full text-left relative" ref={wrapperRef}>
      ...
      {/* Dropdown */}
      {isOpen && !disabled && (
        <div className="absolute top-full left-0 mt-1 w-full z-50 max-h-60 overflow-y-auto rounded-md border border-zinc-200 bg-white py-1 shadow-lg shadow-zinc-900/5 ring-1 ring-zinc-900/5 focus:outline-none">
            ...
            <ul className="text-sm text-zinc-900 pb-1">
              {filteredOptions.map((opt, idx) => (
                <li
                  ...
                  className={`cursor-pointer select-none px-3.5 py-2 hover:bg-zinc-100 flex items-center justify-between ${
                    opt === value ? "bg-zinc-50 font-medium" : ""
                  }`}
                >
              ...
              {inputValue && !filteredOptions.some(opt => opt.toLowerCase() === inputValue.toLowerCase()) && (
                <li
                  ...
                  className="cursor-pointer select-none px-3.5 py-2 mt-1 border-t border-zinc-100 text-indigo-600 hover:bg-indigo-50 font-medium flex items-center"
                >
                  Use &quot;{inputValue}&quot;
                </li>
              )}
