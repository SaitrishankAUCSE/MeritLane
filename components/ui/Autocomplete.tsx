    ...
    <div className="flex flex-col gap-1.5 w-full text-left relative" ref={wrapperRef}>
      ...
      <div className="relative">
        <input
          ...
          className={`w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-500 transition-all duration-150 ease-out hover:border-slate-400 focus:border-[#1a56db] focus:outline-none focus:ring-1 focus:ring-[#1a56db] disabled:opacity-50 disabled:bg-slate-50 ${
            error
      ...
      {/* Dropdown */}
      {isOpen && !disabled && (
        <div className="absolute top-full left-0 mt-1 w-full z-50 max-h-60 overflow-y-auto rounded-md border border-slate-200 bg-white py-1 shadow-lg focus:outline-none">
          {loading && filteredOptions.length === 0 ? (
            ...
            <ul className="text-sm text-slate-900 pb-1">
              {filteredOptions.map((opt, idx) => (
                <li
                  ...
                  className={`cursor-pointer select-none px-3.5 py-2 hover:bg-slate-50 flex items-center justify-between ${
                    opt === value ? "bg-slate-100 font-medium" : ""
                  }`}
                >
                  <span className="truncate">{opt}</span>
                  {opt === value && <Check className="h-4 w-4 text-slate-900 shrink-0 ml-2" />}
                </li>
              ))}
              {inputValue && !filteredOptions.some(opt => opt.toLowerCase() === inputValue.toLowerCase()) && (
                <li
                  ...
                  className="cursor-pointer select-none px-3.5 py-2 mt-1 border-t border-slate-100 text-slate-900 hover:bg-slate-50 font-medium flex items-center"
                >
                  Use &quot;{inputValue}&quot;
                </li>
              )}
