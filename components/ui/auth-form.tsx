        ...
        <div className="relative z-10 my-auto max-w-sm mx-auto w-full">
          <div className="mb-4 border border-[#E7E2DA] bg-white p-1.5 shadow-xs">
            <div className="relative aspect-[16/11] overflow-hidden bg-[#1C1917]">
              ...
              <div className="absolute bottom-2 left-2.5 right-2.5 text-[9px] font-mono text-white/90 uppercase tracking-widest flex items-center justify-between">
                <span>Verification Archive</span>
                <span className="text-white/70">Registry Standard</span>
              </div>
            </div>
          </div>

          <h1 className="text-xl lg:text-[22px] font-serif font-normal tracking-tight leading-[1.2] mb-1.5 text-[#1C1917]">
            Verified Engineering Provenance
          </h1>
          <p className="text-[12px] text-[#78716C] leading-relaxed font-sans">
            Immutable records of verified software engineering capability and code provenance.
          </p>
        </div>

        <div className="relative z-10 text-[10px] font-mono text-[#A8A29E] tracking-wider uppercase shrink-0">
          Standard of Technical Proof
        </div>
      </div>
              ...
              >
                ...
                <div className="grid grid-cols-2 border border-[#E7E2DA] bg-[#FAF8F5] p-0.5 rounded">
                  <button
                    ...
                    className={`text-xs py-1.5 rounded font-mono tracking-wider transition-all ${mode === 'login' ? 'bg-white text-[#1C1917] font-semibold shadow-xs border border-[#E7E2DA]' : 'text-[#78716C] hover:text-[#1C1917]'}`}
                  >
                    SIGN IN
                  </button>
                  <button
                    ...
                    className={`text-xs py-1.5 rounded font-mono tracking-wider transition-all ${mode === 'signup' ? 'bg-white text-[#1C1917] font-semibold shadow-xs border border-[#E7E2DA]' : 'text-[#78716C] hover:text-[#1C1917]'}`}
                  >
                    REGISTER
                  </button>
                </div>
                ...
                {error && (
                  <div className="p-2 bg-red-50 border border-red-100 text-red-600 text-xs rounded">
                    {error}
                  </div>
                )}
                ...
                <button
                  ...
                  disabled={loadingAction}
                  className="w-full flex items-center justify-center gap-2.5 bg-white text-[#1C1917] border border-[#E7E2DA] hover:border-[#1C1917] rounded py-2 px-3 hover:bg-[#FAF8F5] focus:outline-none transition-colors disabled:opacity-50 text-xs font-mono font-medium shadow-2xs"
                >
                ...
                <div className="flex items-center gap-3 py-0.5">
                  ...
                  <span className="text-[9px] font-mono font-medium text-[#78716C] uppercase tracking-widest">OR VIA EMAIL</span>
                  <div className="flex-1 h-px bg-[#E7E2DA]"></div>
                </div>
                ...
                <form onSubmit={handleEmailAuth} className="space-y-3">
                  {mode === "signup" && (
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-mono uppercase tracking-[0.14em] text-[#78716C] block">Role</label>
                      <div className="flex gap-2.5">
                        <label className={`flex-1 relative border rounded p-2 cursor-pointer transition-all ${selectedRole === 'candidate' ? 'border-[#1C1917] bg-[#FAF8F5]' : 'border-[#E7E2DA] hover:border-[#1C1917]'}`}>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-medium text-[#1C1917]">Candidate</span>
                          </div>
                        </label>
                        <label className={`flex-1 relative border rounded p-2 cursor-pointer transition-all ${selectedRole === 'employer' ? 'border-[#1C1917] bg-[#FAF8F5]' : 'border-[#E7E2DA] hover:border-[#1C1917]'}`}>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-medium text-[#1C1917]">Employer</span>
                          </div>
                        </label>
                      </div>
                    </div>
                  ...
                  <div>
                    <label className="text-[10px] font-mono uppercase tracking-[0.14em] text-[#78716C] block mb-1">Email</label>
                    <input 
                      type="email" 
                      required 
                      value={email} 
                    ...
                    {emailStatus.suggestion && (
                      <button
                        ...
                        className="mt-1 text-[11px] text-amber-700 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded hover:bg-amber-100 transition-colors flex items-center gap-1 text-left font-mono"
                      >
                  ...
                  <button 
                    type="submit" 
                    disabled={loadingAction} 
                    className="w-full bg-[#064E3B] hover:bg-[#043327] text-white rounded py-2.5 px-4 text-xs font-mono font-semibold tracking-wider uppercase transition-colors shadow-xs disabled:opacity-50 mt-2 flex items-center justify-center gap-2"
                  >
                    {loadingAction ? (
                      <>
                ...
                <p className="text-[10px] font-mono text-center text-[#78716C] mt-3">
                  Protected by Meritlane Technical Registry.
                </p>
              </motion.div>
            )}
          </AnimatePresence>
